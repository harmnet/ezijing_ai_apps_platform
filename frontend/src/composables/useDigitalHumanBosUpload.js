...
export default function useDigitalHumanBosUpload(data) {
  ...
  const uploadImageWithRetry = async (file, type, onUploadProgress) => {
    ...
    try {
      // 动态导入以避免循环依赖
      const { uploadImageToBos } = await import('../utils/bosDirectUpload');
      ...
      while (retryCount <= MAX_RETRIES) {
        try {
          ...
          // 使用直接上传到百度云BOS的方法
          const imageUrl = await uploadImageToBos(file, type, onUploadProgress);
      ...
      console.error('导入bosDirectUpload模块失败:', error);
      throw new Error('导入上传模块失败: ' + (error.message || '未知错误'));
    }
        ...
        try {
          ...
          if (formData && formData.value) {
            ...
            try {
              ...
              console.error('上传图片到百度云BOS失败:', uploadError);
  ...
  return {
    ...
    // 工具函数
    uploadImageWithRetry,  // 暴露这个方法，以便在其他地方复用
    processUploadedUrl,
