    ...
    <div class="main-layout">
      ...
      <div class="left-panel">
        ...
        <div class="form-group">
          <label for="image-upload" class="required">上传参考图片</label>
          <div class="image-preview-container">
            <img v-if="imagePreview" :src="imagePreview" class="image-preview">
            <div v-else class="image-preview-text">
              <i class="ri-image-line icon-red"></i>
              未选择图片
            </div>
            <button v-if="imagePreview" @click="removeImage" class="remove-image-btn">
          ...
          <small class="form-text">支持JPG、PNG格式，图片将作为视频生成的基础素材</small>
        </div>
        ...
        <div class="form-group">
          ...
          <textarea 
            ...
            placeholder="请输入详细的视频描述，描述您希望图片如何运动或变化，例如：'镜头从近到远，缓慢展现整个场景，逐渐增加光效'"
            class="form-control"
            rows="5"
          ></textarea>
          <small class="form-text">提示词用于指导图片如何变为视频，可描述镜头移动、光效变化等</small>
        </div>
        ...
        <div class="usage-tips">
          ...
          <ul>
            <li>请先上传一张高质量的图片作为视频生成的基础</li>
            <li>文本提示词用于描述图片应该如何变成动态视频</li>
        ...
        <div class="action-buttons">
          <button @click="generateVideo" class="primary-button" :disabled="isLoading || !formData.prompt || !imageBase64">
    ...
    <div class="modal" v-if="showTipsModal">
      <div class="modal-content tips-modal">
        ...
        <div class="modal-body">
          <h4>如何制作高质量的图生视频？</h4>
          <ul class="tips-list">
            <li>选择高质量、高分辨率的图片作为基础素材</li>
            <li>提示词应着重描述希望图片如何动起来，如镜头移动、光效变化</li>
            <li>可以描述画面中特定元素的运动方式，如"花朵轻微摇摆"、"云朵缓慢流动"</li>
            <li>尽量选择符合图片主题的视频比例和时长</li>
          </ul>
          <h4>适合图生视频的图片类型：</h4>
          <ul class="tips-list">
            <li>人物肖像：可让人物眨眼、微笑或添加光效</li>
            <li>风景照片：可添加流动的云彩、水波纹或飘动的树叶</li>
            <li>产品图片：可添加360度旋转或细节特写</li>
            <li>插画作品：可分层添加视差效果</li>
            <li>动物照片：可添加微小的动作如眨眼、摇尾巴</li>
          </ul>
          <h4>提示词示例：</h4>
          <div class="example-list">
            <div class="example-item">
              <strong>肖像照片动态化：</strong>
              <p>人物照片缓慢放大，添加柔和的光效从左至右扫过，眼睛轻微眨动，背景增加轻微模糊和景深变化，使人物形象更加立体</p>
            </div>
            <div class="example-item">
              <strong>风景照片动态化：</strong>
              <p>画面中的云朵缓慢流动，水面增加波纹效果，树叶随风轻轻摇曳，镜头缓慢向前推进，增强沉浸感</p>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      formData: {
        prompt: '',
        ratio: '16:9',
        duration: 5.0, // 默认时长增加为5秒，更适合图生视频
        fps: 24,       // 默认24帧，更加电影感
        resolution: '720p',
        watermark: false,
        image: null
      ...
      checkInterval: null,
      placeholderText: '在左侧上传图片、填写提示词并点击生成视频，生成完成后将在此处显示',
      showTipsModal: false,
  ...
  methods: {
    ...
    async generateVideo() {
      ...
      this.useAlternativePlayer = false;
      this.showLoadingTips = true;
      this.loadingMessage = '正在创建任务...';
      this.apiLogs = [];
      this.retryCount = 0; // 确保重试计数器重置
      ...
      const requestData = {
        ...
        image_url: this.imageBase64 // 使用base64图片作为image_url
      };
      ...
      try {
        ...
        if (response.data.id) {
          ...
          this.taskStatus = response.data.status || 'queued';
          this.loadingMessage = '任务已创建，正在处理...';
          this.addApiLog(`任务创建成功，ID: ${this.taskId}`);
          console.log('成功创建任务，ID:', this.taskId);
          
          // 立即添加到历史记录，状态为queued
          this.saveToHistory(this.taskStatus);
          
          // 开始定期检查任务状态
          this.startPolling();
        }
        // 2. 其他格式
        else if (response.data.status === 'success' || response.data.status === 'running') {
          // 兼容其他可能的成功响应格式
          this.taskId = response.data.task_id || response.data.id;
